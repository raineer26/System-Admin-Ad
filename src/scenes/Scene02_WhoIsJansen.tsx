...
export const Scene02_WhoIsJansen: React.FC<Scene02Props> = ({
  ...
  // Continuous cinematic pan across digital workspace across full 960 frames
  const panX = interpolate(frame, [0, 960], [60, -80], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const cameraScale = interpolate(frame, [0, 960], [1.02, 1.08], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // Smooth cross-transition between Quote and Role cards around frame 510-550
  const quoteExitProgress = interpolate(frame, [500, 540], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const quoteOpacity = 1 - quoteExitProgress;
  const quoteScale = interpolate(quoteExitProgress, [0, 1], [1, 0.92]);
  const quoteShiftX = interpolate(quoteExitProgress, [0, 1], [0, -80]);

  const cardsEntranceProgress = interpolate(frame, [525, 555], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  ...
  return (
    <SceneTransitionWrapper durationInFrames={960} transitionType="slide-left" accentColor={THEME.colors.accentBlue}>
      <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
        ...
        <Background accentColor={THEME.colors.accentBlue} gridOpacity={0.04} glowIntensity={0.25}>
          ...
          {/* Phase 1: Quote Block with smooth spring entrance & exit */}
          {frame < 545 && (
            <div
              style={{
                transform: `translateX(${panX * 0.6 + quoteShiftX}px) scale(${quoteScale})`,
          ...
          {/* Phase 2: Role Cards with spring pop & active continuous floating */}
          {frame >= 525 && (
            <div
              style={{
                display: "flex",
              ...
              <div style={{ display: "flex", gap: "28px" }}>
                {roles.map((role, idx) => {
                  ...
                  return (
                    ...
                    >
                      <ShimmerSweep delay={idx * 20} period={100} color={`${role.color}44`} />
                      <div
                        style={{
                          width: "68px",
                          height: "68px",
